				...
				<div className="space-y-10">
							...
							{article.keywords && article.keywords.length > 0 && (
								<div className="flex flex-wrap gap-2 pt-2">
									{article.keywords.map((keyword, index) => (
										<span
											key={index}
											className="px-3 py-1 rounded-full bg-purple-100 text-violet-700 text-sm font-medium"
										>
											#{keyword}
										</span>
									))}
					...
					<section className="space-y-6">
						...
						<div className="grid grid-cols-1 gap-4">
							{article.relatedArticles.map((related: { title: string; description: string; url: string }, index: number) => (
								<div
									key={index}
					...
					<section className="space-y-6 pb-12">
						...
						<div className="space-y-3">
							{article.relatedPolicies.map((policy: { title: string; description: string; url: string }, index: number) => (
								<div
									key={index}
